...
const XiuxianAlert: React.FC<XiuxianAlertProps> = ({
  ...
  return (
    <div
      className={`mb-6 bg-gradient-to-r ${typeStyles[type]} border rounded-xl p-4 ${className}`}
    >
      <div className="flex items-start space-x-3">
        <div
          className={`w-3 h-3 ${dotColors[type]} rounded-full flex-shrink-0 mt-1`}
        ></div>
        <div className="min-w-0 flex-1">
          <h3
            className={`font-semibold text-sm sm:text-base ${typeStyles[type].split(' ')[3]}`}
          >
            {title}
          </h3>
