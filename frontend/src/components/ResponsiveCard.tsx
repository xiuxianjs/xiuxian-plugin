    ...
    <div className='bg-white rounded-lg sm:rounded-xl lg:rounded-2xl p-4 sm:p-6 shadow-sm hover:shadow-md transition-all duration-200 border border-gray-100'>
      <div className='flex items-start justify-between'>
        <div className='flex-1 min-w-0'>
          <h3 className='text-xs sm:text-sm text-gray-600 font-medium mb-2'>{title}</h3>
          <div className='flex items-baseline space-x-2'>
            {prefix && <span className='text-lg sm:text-xl text-gray-400'>{prefix}</span>}
            <span className='text-lg sm:text-xl lg:text-2xl font-bold' style={{ color }}>
              {value}
            </span>
            {suffix && <span className='text-xs sm:text-sm text-gray-500'>{suffix}</span>}
          </div>
          {description && <p className='text-xs sm:text-sm text-gray-500 mt-2'>{description}</p>}
        </div>
        {trend && (
          <div
            className={classNames('flex items-center space-x-1 px-2 py-1 rounded-full text-xs', {
    ...
    <div className='bg-white rounded-lg sm:rounded-xl lg:rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 border border-gray-100'>
      {title && (
        <div className='px-4 sm:px-6 py-4 border-b border-gray-100'>
          <h3 className='text-sm sm:text-base lg:text-lg font-medium text-gray-900'>{title}</h3>
        </div>
      )}
...
export function ResponsiveButtonGroup({ children, className = '' }: ResponsiveButtonGroupProps) {
  return <div className={classNames('flex flex-wrap gap-2 sm:gap-3', className)}>{children}</div>;
}
