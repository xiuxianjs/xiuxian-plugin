...
export default function DataEditModal({ visible, onCancel, onSuccess, dataType, dataTypeName, originalData }: DataEditModalProps) {
  ...
  // 计算当前页数据
  const currentPageData = editingData.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  ...
  // 渲染字段输入组件
  const renderFieldInput = (field: FieldConfig, value: unknown, index: number) => {
