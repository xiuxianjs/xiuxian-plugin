...
export default function RechargeModal({
  ...
  useEffect(() => {
    if (selectedTier && config?.rechargeTiers) {
      const tierData = config.rechargeTiers.find(
        (tier: any) => tier.name === selectedTier
      )
      if (tierData) {
              ...
              <div className="text-slate-300">
                ...
                <div>
                  ...
                  <span className="text-yellow-400">
                    {selectedUser.currency.toLocaleString()}
                  </span>
                </div>
                <div>
                  总充值:{' '}
                  <span className="text-green-400">
                    ¥{selectedUser.total_recharge_amount}
                  </span>
                </div>
              </div>
              <div className="text-slate-300">
                <div>
                  ...
                  <span className="text-blue-400">
                    {selectedUser.total_recharge_count}
                  </span>
                </div>
                <div>
                  小月卡:{' '}
                  <span className="text-blue-400">
                    {selectedUser.small_month_card_days}天
                  </span>
                </div>
                <div>
                  大月卡:{' '}
                  <span className="text-purple-400">
                    {selectedUser.big_month_card_days}天
                  </span>
                </div>
              </div>
            </div>
                  ...
                  {previewData.firstRechargeBonus && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-pink-400">
                        {previewData.firstRechargeBonus}金币
                      </span>
                    </div>
                  )}
