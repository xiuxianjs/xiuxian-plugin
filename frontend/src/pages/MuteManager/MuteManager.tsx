...
const MuteManager: React.FC = () => {
  ...
  const handleBatchUnmute = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        message.success(
          `批量解除禁言完成，成功解除 ${result.data.successCount} 个用户`
        )
        setSelectedRowKeys([])
        fetchMuteList()
      } else {
  ...
  // 分页处理
  const paginatedData = filteredMuteList.slice(
    (current - 1) * pageSize,
    current * pageSize
  )
    ...
    <XiuxianPageWrapper>
      ...
      >
        ...
        >
          ...
          <Form.Item
            name="reason"
            label={<span className="text-slate-300">禁言原因</span>}
          >
            <Input.TextArea
