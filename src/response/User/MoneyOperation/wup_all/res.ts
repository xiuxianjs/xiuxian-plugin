...
export default onResponse(selects, async e => {
  ...
  if (!body) {
    Send(
      Text('格式: 全体发灵石*数量 / 全体发物品名*数量 / 全体发装备名*品质*数量')
    )
    return false
  }
  const seg = body
  ...
  if (isResource) {
    ...
    for (const qq of playerList) {
      if (name === '灵石') await addCoin(qq, amt)
      else if (name === '修为') await addExp(qq, amt)
      else await addExp2(qq, amt)
    }
