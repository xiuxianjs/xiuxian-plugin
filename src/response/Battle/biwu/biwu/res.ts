...
async function battle(e, num: number) {
  ...
  while (A_player.当前血量 > 0 && B_player.当前血量 > 0) {
    ...
    applyBuffDecay(
      ...
      roundMsgs,
      `${A_player.名号}因为四象封印之力，技能失效,剩余回合{left}`
    )
    applyBuffDecay(
      buff_B,
      A_player,
    ...
    const A_faqiu = Math.trunc(
      A_player.攻击 * Number(A_player.灵根?.法球倍率 || 0)
    )
    ...
    applyBuffDecay(
      ...
      roundMsgs,
      `${B_player.名号}因为四象封印之力，技能失效,剩余回合{left}`
    )
    applyBuffDecay(
      buff_A,
      B_player,
    ...
    const B_faqiu = Math.trunc(
      B_player.攻击 * Number(B_player.灵根?.法球倍率 || 0)
    )
