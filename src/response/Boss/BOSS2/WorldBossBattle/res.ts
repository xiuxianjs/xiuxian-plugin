...
const res = onResponse(selects, async e => {
  ...
  const player = (await getDataJSONParseByKey(keys.player(userId))) as any;
  ...
  const action = (await getDataJSONParseByKey(keysAction.action(userId))) as any;
  ...
  const statusStr = (await getDataJSONParseByKey(KEY_WORLD_BOOS_STATUS_TWO)) as WorldBossStatusInfo;
  const recordStr = (await getDataJSONParseByKey(KEY_RECORD_TWO)) as PlayerRecordData;
