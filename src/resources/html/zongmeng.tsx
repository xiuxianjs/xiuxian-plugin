...
const ZongMeng = ({ temp }: { temp?: ZongMengItem[] }) => {
  ...
  return (
    <HTML>
      <BackgroundImage className='w-full text-white/90 p-6 bg-center bg-cover font-serif' src={celestialSectsURL}>
        {/* 整体背景与布局 */}
        <div className='bg-white bg-opacity-30 p-2 rounded-md backdrop-blur-sm'>
          <main className='max-w-6xl mx-auto space-y-8'>
            ...
            <header className='flex flex-col items-center text-center'>
              ...
              <p className='mt-2 text-base text-sky-800/80'>于此方天地，寻觅汝之道途</p>
            </header>
                ...
                data.map((item, index) => (
                  ...
                  >
                    ...
                    <header className='p-4 border-b border-sky-500/20 flex justify-between items-center'>
                      <h2 className='text-xl font-semibold text-amber-100 tracking-wider'>{item.宗名}</h2>
                      <span className='px-3 py-1 text-sm font-bold bg-sky-800/80 border border-sky-600 rounded-md text-sky-200'>Lv.{item.等级}</span>
                    </header>

                    {/* 卡片主体: 核心信息 */}
                    <div className='p-4 space-y-3 text-sm flex-grow'>
                      <div className='flex items-center gap-3'>
                        <span className='text-sky-300 w-20 shrink-0'> {/* Icon Placeholder */} 仙盟人数:</span>
                        <span className='font-semibold text-white'>
                          {item.人数} / {item.宗门人数上限}
                        </span>
                      </div>
                      <div className='flex items-center gap-3'>
                        <span className='text-sky-300 w-20 shrink-0'> {/* Icon Placeholder */} 天赋加成:</span>
                        <span className='font-semibold text-green-300'>{item.天赋加成}%</span>
                      </div>
                      <div className='flex items-center gap-3'>
                        <span className='text-sky-300 w-20 shrink-0'> {/* Icon Placeholder */} 镇宗神兽:</span>
                        <span className='font-semibold text-white'>{item.镇宗神兽}</span>
                      </div>
                      <div className='flex items-center gap-3'>
                        <span className='text-sky-300 w-20 shrink-0'> {/* Icon Placeholder */} 宗门驻地:</span>
                        <span className='font-semibold text-white'>{item.宗门驻地}</span>
                      </div>
                      <div className='flex items-center gap-3'>
                        <span className='text-sky-300 w-20 shrink-0'> {/* Icon Placeholder */} 加入门槛:</span>
                        <span className='font-semibold text-red-300'>{item.最低加入境界}</span>
                      </div>
                    </div>
                    ...
                    <footer className='mt-auto p-3 bg-slate-950/50 rounded-b-xl border-t border-sky-500/20 flex items-center justify-between'>
                      ...
                      <div className='flex items-center gap-2'>
                        <Image src={getAvatar(item.宗主)} alt='宗主头像' className='w-6 h-6 rounded-full border border-amber-300/50' />
                        <span className='font-mono text-amber-200 text-sm'>{item.宗主}</span>
                      </div>
                    </footer>
                  </article>
                ))
              ) : (
                <p className='col-span-full text-center text-sky-200/70 text-lg'>混沌未开，暂无仙门</p>
              )}
            </section>
          </main>
        </div>
