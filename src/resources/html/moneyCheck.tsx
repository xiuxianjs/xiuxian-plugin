...
const MoneyCheck = ({
  ...
  return (
    <HTML>
      <BackgroundImage className=' w-full flex items-center justify-center p-6 md:p-12 relative bg-center bg-cover' src={ningmenghomeBgURL}>
        {/* 白色居中渐变叠加 */}
        ...
        >
          ...
          <div className='w-full grid grid-cols-2 gap-6 text-blue-900 font-semibold text-xl'>
            <div className='flex flex-col items-center gap-1 p-4 rounded-xl bg-white/40 backdrop-blur-sm shadow-md border border-white/40'>
              ...
              <div className='text-3xl font-extrabold text-blue-700'>{victory}</div>
              <div className='text-xs text-blue-600 mt-1'>共卷走灵石</div>
              <div className='text-2xl font-bold text-blue-800'>{victory_num}</div>
            </div>

            <div className='flex flex-col items-center gap-1 p-4 rounded-xl bg-white/40 backdrop-blur-sm shadow-md border border-white/40'>
              <div className='uppercase tracking-wider text-sm'>败场</div>
              <div className='text-3xl font-extrabold text-red-600'>{defeated}</div>
              <div className='text-xs text-red-500 mt-1'>共献祭灵石</div>
              <div className='text-2xl font-bold text-red-700'>{defeated_num}</div>
            </div>
          </div>
        </main>
      </BackgroundImage>
