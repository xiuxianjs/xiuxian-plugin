  ...
  return (
    <HTML linkStyleSheets={[tempStyles]}>
      <div className='min-h-screen bg-gradient-to-b from-amber-50 via-yellow-100 to-orange-200 flex flex-col items-center py-8 relative overflow-hidden'>
        ...
        <div className='absolute top-0 left-0 w-full h-32 bg-top-fade'></div>
        <div className='absolute bottom-0 left-0 w-full h-32 bg-bottom-fade'></div>
              ...
              temp.map((item, index) => (
                <div key={index} className='relative group'>
                  ...
                  <div className='xiuxian-message bg-gradient-to-br from-white/90 via-amber-50/95 to-orange-50/90 backdrop-blur-md rounded-2xl px-4 py-3 shadow-2xl border border-amber-300/50 overflow-hidden'>
                    {/* 左上角装饰 */}
                    <div className='corner-decoration top-0 left-0 w-16 h-16 rounded-br-2xl'></div>

                    {/* 右上角装饰 */}
                    <div className='corner-decoration top-0 right-0 w-12 h-12 rounded-bl-2xl'></div>

                    {/* 左下角装饰 */}
                    <div className='corner-decoration bottom-0 left-0 w-10 h-10 rounded-tr-2xl'></div>

                    {/* 右下角装饰 */}
                    <div className='corner-decoration bottom-0 right-0 w-14 h-14 rounded-tl-2xl'></div>
                    ...
                    {/* 底部装饰线 */}
                    <div className='absolute bottom-2 left-1/2 transform -translate-x-1/2 w-3/4 h-0.5 bg-gradient-to-r from-transparent via-amber-400/60 to-transparent rounded-full'></div>
                  </div>

                  {/* 连接线（除了最后一个） */}
                  {index < temp.length - 1 && (
                    <div className='connection-line absolute left-6 top-full w-0.5 h-6'></div>
                  )}
                </div>
              ))}
          </div>
