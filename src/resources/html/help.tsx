...
const Help = ({ helpData = [], page = 1, pageSize, total }) => {
  return (
    <HTML linkStyleSheets={[saURL]}>
      <BackgroundImage className='bg-[length:100%_100%] bg-center  elem-default default-mode text-[18px] text-brand font-sans ' src={[backgroundURL]}>
        {/* 静态背景效果 */}
        <div className='absolute inset-0 pointer-events-none'>
          ...
          <div className='absolute top-32 right-24 text-purple-400/15 text-2xl'>⚡</div>
          <div className='absolute bottom-32 left-20 text-yellow-400/15 text-2xl'>🌟</div>
          <div className='absolute bottom-16 right-16 text-pink-400/15 text-3xl'>💫</div>
        </div>

        <div className='relative z-10 max-w-[1000px] mx-auto px-6 py-10' id='container'>
          {/* 顶部标题区域 */}
                          ...
                          <div className='relative flex items-start gap-3'>
                            ...
                            <div className='flex-shrink-0'>
                              <div className='w-12 h-12 rounded-xl bg-gradient-to-br from-teal-500/20 to-cyan-500/20 border border-teal-400/30 flex items-center justify-center shadow-lg'>
                                <span className='text-2xl'>{getCommandIcon(item.title)}</span>
                              </div>
                            </div>
                  ...
                  <div className='inline-flex items-center gap-3 bg-gradient-to-r from-red-500/20 to-yellow-500/20 px-6 py-3 rounded-2xl shadow-inner border border-red-400/30 backdrop-blur-sm'>
                    ...
                    {typeof total === 'number' && total > 0 ? (
                      <>
                        ...
                        <b className='text-2xl md:text-3xl text-red-300 font-black'>{total}</b>
