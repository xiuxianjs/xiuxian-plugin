        ...
        <div className='w-full max-w-2xl mx-auto'>
          <div className='rounded-xl shadow-lg bg-white p-6 mb-6 flex flex-col items-center'>
            ...
            {didian_list?.map((item, index) => (
              <div key={index} className='border rounded-lg p-4 bg-gray-50 hover:bg-gray-100 transition mb-4 w-full'>
                  ...
                  <div>
                    ...
                    <span className='flex flex-wrap gap-2 ml-2'>
                      {item.one?.map((thing, idx) => (
                        <span key={idx} className='inline-block bg-blue-200 text-blue-900 rounded px-2 py-1 text-xs font-semibold'>
                          {thing.name}
                        </span>
                      ))}
                    </span>
                  ...
                  <div>
                    ...
                    <span className='flex flex-wrap gap-2 ml-2'>
                      {item.two?.map((thing, idx) => (
                        <span key={idx} className='inline-block bg-green-200 text-green-900 rounded px-2 py-1 text-xs font-semibold'>
                          {thing.name}
                        </span>
                      ))}
                    </span>
                  ...
                  <div>
                    ...
                    <span className='flex flex-wrap gap-2 ml-2'>
                      {item.three?.map((thing, idx) => (
                        <span key={idx} className='inline-block bg-yellow-200 text-yellow-900 rounded px-2 py-1 text-xs font-semibold'>
                          {thing.name}
                        </span>
                      ))}
                    </span>
