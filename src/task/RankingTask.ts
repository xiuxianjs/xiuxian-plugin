...
import type { AssociationInfo } from '@src/types/model'
...
interface AssociationRanking extends RankingItem {
  extra: {
    宗主: string
    成员数: number
    宗门等级: number
...
interface PlayerRanking extends RankingItem {
  extra: {
    名号: string
    境界: string
    宗门?: string
...
async function calculateAssociationRankings() {
  try {
    ...
    const associations: AssociationRanking[] = []
      ...
      if (associationData) {
        try {
          const association = JSON.parse(
            decodeURIComponent(associationData)
          ) as AssociationInfo
          ...
          associations.push({
            ...
            value:
              power * 1000 + memberCount * 100 + level * 10 + lingshi / 10000,
            rank: 0,
            extra: {
              宗主: association.宗主 || '',
              成员数: memberCount,
    ...
    // 缓存排名数据
    await redis.setex(
      RANKING_KEYS.ASSOCIATION_POWER,
      3600,
      JSON.stringify(associations)
    )

    // 按成员数排序
    const memberRanking = [...associations].sort(
      (a, b) => b.extra.成员数 - a.extra.成员数
    )
    memberRanking.forEach((item, index) => {
      item.rank = index + 1
    })
    await redis.setex(
      RANKING_KEYS.ASSOCIATION_MEMBERS,
      3600,
      JSON.stringify(memberRanking)
    )

    // 按灵石池排序
    const lingshiRanking = [...associations].sort(
      (a, b) => b.extra.灵石池 - a.extra.灵石池
    )
    lingshiRanking.forEach((item, index) => {
      item.rank = index + 1
    })
    await redis.setex(
      RANKING_KEYS.ASSOCIATION_LINGSHI,
      3600,
      JSON.stringify(lingshiRanking)
    )

    // 按宗门等级排序
    const levelRanking = [...associations].sort(
      (a, b) => b.extra.宗门等级 - a.extra.宗门等级
    )
    levelRanking.forEach((item, index) => {
      item.rank = index + 1
    })
    await redis.setex(
      RANKING_KEYS.ASSOCIATION_LEVEL,
      3600,
      JSON.stringify(levelRanking)
    )
...
async function calculatePlayerRankings() {
  try {
    ...
    await redis.setex(
      RANKING_KEYS.PLAYER_LEVEL,
      3600,
      JSON.stringify(levelRanking)
    )

    // 按攻击力排序
    const attackRanking = [...players].sort(
      (a, b) => b.extra.攻击 - a.extra.攻击
    )
    attackRanking.forEach((item, index) => {
      item.rank = index + 1
    })
    await redis.setex(
      RANKING_KEYS.PLAYER_ATTACK,
      3600,
      JSON.stringify(attackRanking)
    )

    // 按防御力排序
    const defenseRanking = [...players].sort(
      (a, b) => b.extra.防御 - a.extra.防御
    )
    defenseRanking.forEach((item, index) => {
      item.rank = index + 1
    })
    await redis.setex(
      RANKING_KEYS.PLAYER_DEFENSE,
      3600,
      JSON.stringify(defenseRanking)
    )
...
export async function getRankingData(
  ...
  try {
    const key = RANKING_KEYS[type as keyof typeof RANKING_KEYS]
    if (!key) {
    ...
    const rankings = JSON.parse(data) as RankingItem[]
...
// 定时任务主函数
export async function runRankingTask() {
  try {
