...
export async function plantJiesuan(user_id: string, time: number, groupId?: string) {
...
export async function mineJiesuan(user_id: string, time: number, groupId?: string) {
  const userId = user_id;
  const player = await getDataJSONParseByKey(keys.player(userId));
