...
import '@alemonjs/db';
import { notUndAndNull, Harm, readShop, addNajieThing, writeShop } from '../model/xiuxian.js';
import 'dayjs';
                ...
                if (now_time >= end_time) {
                    ...
                    if (arr.cishu == 0) {
                        ...
                        for (let j = 0; j < arr.thing.length; j++) {
                            await addNajieThing(player_id, arr.thing[j].name, arr.thing[j].class, arr.thing[j].数量);
                            last_msg += '';
                        }
                        shop[i].Grade++;
                        if (shop[i].Grade > 3) {
