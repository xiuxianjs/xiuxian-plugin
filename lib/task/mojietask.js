...
const MojiTask = async () => {
    const keys$1 = await redis.keys(`${__PATH.player_path}:*`);
    const playerList = keys$1.map(key => key.replace(`${__PATH.player_path}:`, ''));
    for (const player_id of playerList) {
        ...
        if (action != null) {
            ...
            if (String(act.mojie) == '0') {
                const baseDuration = typeof act.time === 'number'
                    ? act.time
