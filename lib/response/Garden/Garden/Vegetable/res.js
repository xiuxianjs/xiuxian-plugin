...
import '@alemonjs/db';
import { notUndAndNull } from '../../../../model/xiuxian.js';
import 'dayjs';
...
var res = onResponse(selects, async (e) => {
    ...
    if (!notUndAndNull(player.宗门)) {
        ...
        await new_Garden(player.宗门.宗门名称, usr_qq);
    ...
    for (let i = 0; i < ass.药园.作物.length; i++) {
        ...
        if (zuowu[i].name == '天灵花' ||
            ...
            zuowu[i].name == '创世花')
            continue;
        let vegetable_Oldtime = await redis.get('xiuxian:' + ass.宗门名称 + zuowu[i].name);
        let chengshu_t = Math.trunc((vegetable_Oldtime - nowTime) / 86400000);
        let chengshu_m = Math.trunc(((vegetable_Oldtime - nowTime) % 86400000) / 60 / 60 / 1000);
        let chengshu_s = Math.trunc(((vegetable_Oldtime - nowTime) % 3600000) / 60 / 1000);
