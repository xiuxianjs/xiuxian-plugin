...
import { Boss2IsAlive, WorldBossBattleInfo, InitWorldBoss2, WorldBossBattle, SetWorldBOSSBattleUnLockTimer, SortPlayer } from '../../boss.js';
...
var res = onResponse(selects, async (e) => {
    ...
    if (await data.existData('player', usr_qq)) {
        ...
        if (WorldBossBattleInfo.CD[usr_qq]) {
            let Seconds = Math.trunc((300000 - (new Date().getTime() - WorldBossBattleInfo.CD[usr_qq])) /
                1000);
        ...
        if (WorldBossBattleInfo.Lock != 0) {
            Send(Text('好像有人正在和银角大王激战，现在去怕是有未知的凶险，还是等等吧！'));
            return false;
        }
        ...
        if (msg.find(item => item == A_win)) {
            TotalDamage = Math.trunc(WorldBossStatus.Healthmax * 0.06 +
                Harm(player.攻击 * 0.85, Boss.防御) * 10);
            WorldBossStatus.Health -= TotalDamage;
            Send(Text(`${player.名号}击败了[${Boss.名号}],重创[金角大王],造成伤害${TotalDamage}`));
        }
        else if (msg.find(item => item == B_win)) {
            TotalDamage = Math.trunc(WorldBossStatus.Healthmax * 0.04 +
                Harm(player.攻击 * 0.85, Boss.防御) * 6);
            WorldBossStatus.Health -= TotalDamage;
            Send(Text(`${player.名号}被[${Boss.名号}]击败了,只对[金角大王]造成了${TotalDamage}伤害`));
        }
        ...
        if (random < 0.05 && msg.find(item => item == A_win)) {
            ...
            Send(Text(`危及时刻,万先盟-韩立前来助阵,对[金角大王]造成${Math.trunc(WorldBossStatus.Health * 0.15)}伤害,并治愈了你的伤势`));
        ...
        if (WorldBossStatus.Health <= 0) {
            ...
            for (let i = 0; i < PlayerList.length; i++) {
                let CurrentPlayer = await await data.getData('player', PlayerRecordJSON.QQ[PlayerList[i]]);
                if (i < Show_MAX) {
                    ...
                    logger.info(`[金角大王周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励${Reward}`);
                    continue;
                }
                else {
                    CurrentPlayer.灵石 += 200000;
                    logger.info(`[金角大王周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励200000`);
                    data.setData('player', PlayerRecordJSON.QQ[PlayerList[i]], CurrentPlayer);
                }
