...
import { KEY_WORLD_BOOS_STATUS_TWO, KEY_RECORD_TWO, KEY_AUCTION_GROUP_LIST } from '../../../../model/constants.js';
...
var res = onResponse(selects, async (e) => {
    ...
    const WorldBossStatus = parseJson(statusStr, null);
    if (!WorldBossStatus) {
    ...
    if (!recordStr || recordStr === '0') {
        ...
        PlayerRecordJSON = parseJson(recordStr, {
            QQ: [],
            TotalDamage: [],
            Name: []
        });
    ...
    if (playerWin) {
        dealt = Math.trunc(WorldBossStatus.Healthmax * 0.06 +
            Harm(player.攻击 * 0.85, Boss.防御) * 10);
        WorldBossStatus.Health -= dealt;
        Send(Text(`${player.名号}击败了[${Boss.名号}],重创[金角大王],造成伤害${dealt}`));
    }
    else if (bossWin) {
        dealt = Math.trunc(WorldBossStatus.Healthmax * 0.04 + Harm(player.攻击 * 0.85, Boss.防御) * 6);
        WorldBossStatus.Health -= dealt;
        Send(Text(`${player.名号}被[${Boss.名号}]击败了,只对[金角大王]造成了${dealt}伤害`));
    }
