...
global.WorldBOSSBattleUnLockTimer = 0;
        ...
        if (WorldBossStatus.Health <= 0) {
            ...
            for (let i = 0; i < PlayerList.length; i++) {
                ...
                if (i < Show_MAX) {
                    ...
                    console.log(`[妖王周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励${Reward}`);
                    continue;
                }
                else {
                    CurrentPlayer.灵石 += 200000;
                    console.log(`[妖王周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励200000`);
                    data.setData('player', PlayerRecordJSON.QQ[PlayerList[i]], CurrentPlayer);
                }
...
export { BossIsAlive, GetAverageDamage, InitWorldBoss, LookUpWorldBossStatus, SetWorldBOSSBattleUnLockTimer, SortPlayer, WorldBossBattle };
