...
var res = onResponse(selects, async (e) => {
    ...
    const pool = parseJson(await redis.get('xiuxian@1.3.0:1:shangjing'));
    ...
    const targetAction = parseJson(await redis.get(`xiuxian@1.3.0:${qq}:action`));
    if (targetAction) {
        const now = Date.now();
    ...
    const buff = player.occupation === '侠客'
        ? 1 + (player.occupation_level || 0) * 0.055
        : 1;
