...
import { getRedisKey, __PATH } from '../../../../model/keys.js';
...
async function applyRebirthCommon(usr_qq, player) {
    player.level_id = 9;
...
async function exitAssociationIfNeed(usr_qq, player, Send) {
    ...
    if (!isAssociation(assRaw) || !assRaw.power)
        return;
    ...
    if (guild.职位 !== '宗主') {
        ...
        if (isAssociation(ass2Raw)) {
            ...
            if (Array.isArray(ass2[guild.职位]))
                ass2[guild.职位] = ass2[guild.职位].filter(q => q !== usr_qq);
            if (Array.isArray(ass2['所有成员']))
                ass2['所有成员'] = ass2['所有成员'].filter(q => q !== usr_qq);
            data.setAssociation(ass2Raw.宗门名称, ass2Raw);
        }
        delete player.宗门;
        await writePlayer(usr_qq, player);
    ...
    if (randmember_qq) {
        ...
        if (randmember &&
            ...
            if (Array.isArray(arr))
                ass3[pos] = arr.filter(q => q !== randmember_qq);
            ass3['宗主'] = randmember_qq;
            randmember.宗门.职位 = '宗主';
            await writePlayer(randmember_qq, randmember);
        }
    }
    ass3['所有成员'] = ass3['所有成员'].filter(q => q !== usr_qq);
    delete player.宗门;
    await writePlayer(usr_qq, player);
    data.setAssociation(ass3.宗门名称, ass3);
    await playerEfficiency(usr_qq);
    Send(Text(`轮回前,遵循你的嘱托,${randmember_qq}将继承你的衣钵,成为新一任的宗主`));
}
const FAIL_PROB = 1 / 9;
const numVal = (v, d = 0) => typeof v === 'number' && !isNaN(v)
    ? v
