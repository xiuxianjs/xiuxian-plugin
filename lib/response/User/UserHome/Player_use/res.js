import { useMessage, Text, useSend, Image, useSubscribe, Mention } from 'alemonjs';
import '../../../../model/api.js';
import '@alemonjs/db';
...
import { foundthing, getRandomTalent } from '../../../../model/cultivation.js';
import { existNajieThing, insteadEquipment, addNajieThing } from '../../../../model/najie.js';
import { addHP, addExp, addExp2, addExp3 } from '../../../../model/economy.js';
import { readEquipment, writeEquipment } from '../../../../model/equipment.js';
import { playerEfficiency } from '../../../../model/efficiency.js';
import { existplayer, readPlayer, readNajie } from '../../../../model/xiuxian_impl.js';
import { readDanyao, writeDanyao } from '../../../../model/danyao.js';
import { mapDanyaoArrayToStatus } from '../../../../model/utils/danyao.js';
...
const thingType = (obj) => obj && typeof obj === 'object' && 'type' in obj
    ? obj.type
    : undefined;
var res = onResponse(selects, async (e) => {
    ...
    const x = await existNajieThing(usr_qq, thing_name, (thingClass || '道具'), pinji);
    if (!x) {
        message.send(format(Text(`你没有【${thing_name}】这样的【${thingClass || '未知类别'}】`)));
        return;
    }
    ...
    if (func[0] == '服用') {
        ...
        if (tType === '锻造上限') {
            ...
            message.send(format(Text(`服用成功,您下一次的炼器获得了额外的炼器格子[${dy.beiyong5}]`)));
            await addNajieThing(usr_qq, thing_name, '丹药', -quantity);
            await writeDy();
            return;
        }
        ...
        if (tType === '入魔') {
            const md = numOr('modao');
            await addExp3(usr_qq, quantity * md);
            message.send(format(Text(`${quantity}道黑色魔气入体,增加了${quantity * md}魔道值`)));
            await addNajieThing(usr_qq, thing_name, '丹药', -quantity);
            return;
        }
        if (tType === '补根') {
    ...
    if (func[0] == '消耗') {
        if (thing_name == '轮回阵旗') {
            player.lunhuiBH = 1;
            await writePlayer(usr_qq, player);
            message.send(format(Text('已得到"轮回阵旗"的辅助，下次轮回可抵御轮回之苦的十之八九')));
        ...
        if (thing_name == '重铸石') {
            ...
            for (const t of type) {
                ...
                if (!cur || cur.pinji === undefined || !z[cur.pinji])
                    continue;
        ...
        if (thing_exist && thingType(thing_exist) == '洗髓') {
            ...
            message.send(format(Mention(usr_qq), Text([
                `  服用成功,剩余 ${thing_name}数量: ${(typeof numberOwned === 'number' ? numberOwned : 0) - 1}，新的灵根为 "${player.灵根.type}"：${player.灵根.name}`,
                '\n可以在【#我的练气】中查看'
            ].join(''))));
            return;
        }
        ...
        if (qh) {
            ...
            player.攻击加成 +=
                toNumber(qh.攻击) * quantity;
            player.防御加成 +=
                toNumber(qh.防御) * quantity;
            player.生命加成 +=
                toNumber(qh.血量) * quantity;
            await writePlayer(usr_qq, player);
