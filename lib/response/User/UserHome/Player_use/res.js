...
import { read_all } from '../../../../model/duanzaofu.js';
import { existplayer, Read_player, Read_najie, convert2integer, foundthing, exist_najie_thing, instead_equipment, get_equipment_img, Read_danyao, isNotNull, Add_HP, Add_najie_thing, Add_修为 as Add___, Add_血气 as Add___$1, Write_danyao, Write_player, Add_魔道值 as Add____, Read_equipment, Write_equipment, get_random_talent, player_efficiency, Add_player_学习功法 as Add_player_____ } from '../../../../model/xiuxian.js';
import data from '../../../../model/XiuxianData.js';

const name = createEventName(import.meta.url);
const selects = onSelects(['message.create', 'private.message.create']);
const regular = /^(#|\/)(装备|消耗|服用|学习|兑换)((.*)|(.*)*(.*))$/;
var res = onResponse(selects, async (e) => {
    ...
    if (func[0] == '服用') {
        ...
        if (thing_exist.type == '幸运') {
            ...
            Send(Text(`${thing_name}服用成功，将在之后的 ${quantity * 10}次冒险旅途中为你提高幸运值！`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            return false;
        }
        if (thing_exist.type == '闭关') {
            ...
            Send(Text(`${thing_name}提高了你的忍耐力,提高了下次闭关的效率,当前提高${dy.biguanxl * 100}%\n查看练气信息后生效`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
        ...
        if (thing_exist.type == '仙缘') {
            ...
            Send(Text(`${thing_name}赐予${player.名号}仙缘,${player.名号}得到了仙兽的祝福`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            await Write_danyao(usr_qq, dy);
            return false;
        }
        if (thing_exist.type == '凝仙') {
            ...
            Send(Text(`丹韵入体,身体内蕴含的仙丹药效增加了${thing_exist.机缘 * quantity}次`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            await Write_danyao(usr_qq, dy);
            return false;
        }
        if (thing_exist.type == '炼神') {
            ...
            Send(Text(`服用了${thing_name},获得了炼神之力,下次闭关获得了炼神之力,当前炼神之力为${thing_exist.lianshen * 100}%`));
            await Write_danyao(usr_qq, dy);
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            return false;
        }
        if (thing_exist.type == '神赐') {
            ...
            Send(Text(`${player.名号}获得了神兽的恩赐,赐福的概率增加了,当前剩余次数${dy.beiyong2}`));
            await Write_danyao(usr_qq, dy);
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            return false;
        }
        if (thing_exist.type == '灵根') {
            ...
            Send(Text(`神药入体,${player.名号}更改了自己的隐藏灵根,当前隐藏灵根为[${player.隐藏灵根.name}]`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -1);
            return false;
        }
        if (thing_exist.type == '器灵') {
            if (!player.锻造天赋) {
                Send(Text(`请先去#炼器师能力评测,再来更改天赋吧`));
                return false;
            }
            player.锻造天赋 = player.锻造天赋 + thing_exist.天赋 * quantity;
            Send(Text(`服用成功,您额外获得了${thing_exist.天赋 * quantity}天赋上限,您当前炼器天赋为${player.锻造天赋}`));
            await Write_player(usr_qq, player);
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            return false;
        }
        if (thing_exist.type == '锻造上限') {
            if (dy.beiyong5 > 0) {
                Send(Text(`您已经增加了锻造上限,消耗完毕再接着服用吧`));
                return false;
            }
            dy.xingyun = quantity;
            dy.beiyong5 = thing_exist.额外数量;
            Send(Text(`服用成功,您下一次的炼器获得了额外的炼器格子[${thing_exist.额外数量}]`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            await Write_danyao(usr_qq, dy);
            return false;
        }
        if (thing_exist.type == '魔道值') {
            ...
            Send(Text(`获得了转生之力,降低了${quantity * thing_exist.modao}魔道值`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            return false;
        }
        if (thing_exist.type == '入魔') {
            await Add____(usr_qq, quantity * thing_exist.modao);
            Send(Text(`${quantity}道黑色魔气入体,增加了${quantity * thing_exist.modao}魔道值`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
            return false;
        }
        if (thing_exist.type == '补根') {
            ...
            data.setData('player', usr_qq, player);
            Send(Text(`服用成功,当前灵根为垃圾五灵根,你具备了称帝资格`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -1);
            return false;
        }
        if (thing_exist.type == '补天') {
            ...
            data.setData('player', usr_qq, player);
            Send(Text(`服用成功,当前灵根为天五灵根,你具备了称帝资格`));
            await Add_najie_thing(usr_qq, thing_name, '丹药', -1);
            return false;
        }
    ...
    if (func[0] == '消耗') {
        if (thing_name == '轮回阵旗') {
            player.lunhuiBH = 1;
            data.setData('player', usr_qq, player);
            Send(Text('已得到"轮回阵旗"的辅助，下次轮回可抵御轮回之苦的十之八九'));
        ...
        if (thing_name == '重铸石') {
            ...
            Send(Text('使用成功,发送#我的装备查看属性'));
            return false;
        }
        if (thing_exist.type == '洗髓') {
            if ((await player.linggenshow) != 0) {
                await Send(Text('你未开灵根，无法洗髓！'));
                return false;
            }
        ...
        if (qh) {
            if (qh.class == '魔头' && player.魔道值 < 1000) {
                Send(Text(`你还是提升点魔道值再用吧!`));
                return false;
            }
            else if (qh.class == '神人' &&
                (player.魔道值 > 0 ||
                    (player.灵根.type != '转生' && player.level_id < 42))) {
                Send(Text(`你尝试使用它,但是失败了`));
                return false;
            }
    ...
    if (func[0] == '学习') {
        ...
        Send(Text(`你学会了${thing_name},可以在【#我的炼体】中查看`));
    }
});
