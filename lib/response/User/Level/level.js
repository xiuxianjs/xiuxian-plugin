...
async function useLevelUp(e, luck = false) {
    ...
    if (rand > prob) {
        ...
        if (bad_time > 0.9) {
            ...
            void Send(Text('突然听到一声鸡叫,鸡..鸡..鸡...鸡你太美！！！是翠翎恐蕈，此地不适合突破，快跑！险些走火入魔，丧失了'
                + needEXP * 0.4
                + '修为'));
            return false;
        }
            ...
            await setTimestamp(userId, 'last_Levelup_time', now_Time);
            void Send(Text('突破瓶颈时想起背后是药园，刚种下掣电树种子，不能被破坏了，打断突破，嘴角流血，丧失了'
                + needEXP * 0.1
                + '修为'));
            return false;
        }
            ...
            await addExp(userId, -1 * needEXP * 0.2);
            await setTimestamp(userId, 'last_Levelup_time', now_Time);
            void Send(Text('突破瓶颈时想起怡红院里的放肆,想起了金银坊里的狂热,险些走火入魔，丧失了'
                + needEXP * 0.2
                + '修为'));
            return false;
        }
    ...
    if (now_level_id < 42) {
        ...
        if (random < ((now_level_id / 60) * 0.5) / 5) {
            ...
            void Send(Text('修仙本是逆天而行,神明愿意降下自己的恩泽.这只['
