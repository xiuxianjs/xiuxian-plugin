...
import { Go, Read_player, sleep, isNotNull, Add_灵石 as Add___, exist_najie_thing, Add_najie_thing, Add_修为 as Add___$1 } from '../../../../model/xiuxian.js';
...
var res = onResponse(selects, async (e) => {
    ...
    let suiji = Math.round(Math.random());
    let yunqi = Math.random();
    await sleep(1000);
    Send(Text('你在冲水堂发现有人上架了一份仙府地图'));
    let didian = didianlist[suiji];
    let now_level_id;
    ...
    let dazhe = 1;
    if ((await exist_najie_thing(usr_qq, '仙府通行证', '道具')) &&
        player.魔道值 < 1 &&
    ...
    await Add___$1(usr_qq, -1e5);
    if (suiji == 0) {
        Send(Text('你买下了那份地图,历经九九八十一天,终于到达了地图上的仙府,洞府上模糊得刻着[' +
            weizhi.name +
    ...
    if (suiji == 1) {
        Send(Text('你买下了那份地图,历经九九八十一天,终于到达了地图上的地点,这座洞府仿佛是上个末法时代某个仙人留下的遗迹,你兴奋地冲进去探索机缘,被强大的仙气压制，消耗了100000修为成功突破封锁闯了进去' +
            time +
            '分钟后归来!'));
    }
});
