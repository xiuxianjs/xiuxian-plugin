...
const PATCH = async (ctx) => {
    try {
        ...
        const { TaskMap } = await import('../../model/task.js');
        const taskStatus = {};
...
const PUT = async (ctx) => {
    try {
        ...
        const { action, taskName } = ctx.request.body;
        const { startSingleTask, stopTask, restartTask, startAllTasks, stopAllTasks, restartAllTasks } = await import('../../task/index.js');
        let success = false;
        let message = '';
        let data = {};
        switch (action) {
