...
const BadgeList = ({ title, items }) => (React.createElement("section", { className: "w-full rounded-2xl bg-white/5 backdrop-blur-md ring-1 ring-white/10 p-4 md:p-6 shadow-card space-y-3" },
    React.createElement("h2", { className: "text-lg md:text-xl font-semibold text-white tracking-wider flex items-center gap-2" },
        React.createElement("span", { className: "w-1.5 h-6 bg-brand-accent rounded-full" }),
        title),
    React.createElement("div", { className: "flex flex-wrap gap-2" }, items?.length ? (items.map((it, i) => (React.createElement("span", { key: i, className: "px-3 py-1 rounded-full bg-brand-dark/60 text-brand-accent text-sm font-medium shadow ring-1 ring-brand-accent/20" }, it)))) : (React.createElement("span", { className: "text-white/60 text-sm" }, "\u6682\u65E0")))));
const InfoItem = ({ label, value }) => (React.createElement("div", { className: "flex items-center justify-between gap-4 px-4 py-2 rounded-lg bg-white/10 text-white text-sm md:text-base" },
    React.createElement("span", { className: "font-medium tracking-wide" }, label),
    React.createElement("span", { className: "font-semibold text-brand-accent break-all" }, value)));
        ...
        React.createElement("main", { className: "max-w-6xl mx-auto space-y-8" },
            React.createElement("header", { className: "flex flex-col md:flex-row md:items-center md:justify-between gap-8" },
                React.createElement("div", { className: "flex flex-col items-center gap-4" },
                    React.createElement("div", { className: "w-56 h-56 rounded-full bg-cover bg-center ring-4 ring-white/30 shadow-card", style: { backgroundImage: `url(${fileUrl$2})` } },
                        React.createElement("img", { className: "w-full h-full rounded-full object-cover mix-blend-luminosity", src: `https://q1.qlogo.cn/g?b=qq&s=0&nk=${user_id}`, alt: "\u5934\u50CF" })),
                    React.createElement("div", { className: "px-5 py-1.5 rounded-2xl bg-black/40 backdrop-blur text-white text-lg font-semibold shadow" },
                        "QQ: ",
                        user_id)),
                React.createElement("div", { className: "flex-1 grid gap-3 md:grid-cols-2 lg:grid-cols-3" },
                    ...
                    React.createElement(InfoItem, { label: "\u5B97\u4E3B", value: `${mainname}${mainqq}` }),
            ...
            React.createElement("section", { className: "w-full rounded-2xl bg-white/5 backdrop-blur-md ring-1 ring-white/10 p-4 md:p-6 shadow-card space-y-4" },
                React.createElement("h2", { className: "text-xl md:text-2xl font-semibold text-white tracking-wider flex items-center gap-2" },
                    React.createElement("span", { className: "w-1.5 h-6 bg-brand-accent rounded-full" }),
                    "\u4FE1\u606F"),
            ...
            React.createElement("section", { className: "w-full rounded-2xl bg-white/5 backdrop-blur-md ring-1 ring-white/10 p-4 md:p-6 shadow-card" },
                React.createElement("p", { className: "text-white/80 text-sm md:text-base tracking-wide" },
                    "\u521B\u7ACB\u4E8E: ",
                    ass?.创立时间?.[0] || '-')))));
};
