...
import { generateCaptcha, svgToPngBuffer, verifyCaptcha } from '../model/captcha.js';
...
var mw = onMiddleware(selects, async (event, next) => {
    ...
    if (captchaExists) {
        ...
        if (success) {
            ...
            await redis.set(hourKey, '30', 'EX', 60 * 60 * 4);
            for (let i = 1; i <= 3; i++) {
            ...
            await redis.set(`${baseKey}:captcha_passed:${userId}`, '1', 'EX', 300);
            next();
            return;
        }
        else {
