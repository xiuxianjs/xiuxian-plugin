...
import { Read_it, Writeit } from './duanzaofu.js';
import { MyDirPath } from '../app.config.js';
import config from './Config.js';
import Game from './show.js';
...
async function Read_player(usr_qq) {
    ...
    const data = JSON.parse(decodeURIComponent(player));
    return data;
}
async function LevelTask(e, power_n, power_m, power_Grade, aconut) {
    const usr_qq = e.UserId;
    const Send = useSend(e);
    const msg = [Number(usr_qq)];
...
async function Read_najie(usr_qq) {
    ...
    let data;
    try {
        data = JSON.parse(najie);
    }
    catch {
        await fixed(usr_qq);
        data = await Read_najie(usr_qq);
    }
    return data;
}
async function get_XianChong_img(e) {
    ...
    const data1 = await new Game().get_xianchong(player_data);
...
async function get_daoju_img(e) {
    ...
    const data1 = await new Game().get_daojuData(player_data);
...
async function get_wuqi_img(e) {
    ...
    const data1 = await new Game().get_wuqiData(player_data);
...
async function get_danyao_img(e) {
    ...
    const data1 = await new Game().get_danyaoData(player_data);
...
async function get_gongfa_img(e) {
    ...
    const data1 = await new Game().get_gongfaData(player_data);
...
async function get_power_img(e) {
    ...
    const data1 = await new Game().get_playercopyData(playercopy);
...
async function get_player_img(e) {
    ...
    equipment.武器.bao = Math.floor(equipment.武器.bao * 100) + '%';
    equipment.护具.bao = Math.floor(equipment.护具.bao * 100) + '%';
    equipment.法宝.bao = Math.floor(equipment.法宝.bao * 100) + '%';
    ...
    let player_data = {
        ...
        expmax_liandan,
        equipment,
        talent: Math.floor(player.修炼效率提升 * 100),
    ...
    const data1 = await new Game().get_playerData(player_data);
...
async function get_association_img(e) {
    ...
    const data1 = await new Game().get_associationData(association_data);
...
async function get_equipment_img(e) {
    ...
    const data1 = await new Game().get_equipmnetData(player_data);
...
async function get_najie_img(e) {
    ...
    const data1 = await new Game().get_najieData(player_data);
...
async function get_state_img(e, all_level) {
    ...
    const data1 = await new Game().get_stateData(state_data);
...
async function get_statezhiye_img(e, all_level) {
    ...
    const data1 = await new Game().get_stateDatazhiye(state_data);
...
async function get_statemax_img(e, all_level) {
    ...
    const data1 = await new Game().get_statemaxData(statemax_data);
...
async function get_talent_img(e) {
    ...
    const data1 = await new Game().get_talentData(talent_data);
...
async function get_adminset_img(e) {
    ...
    const data1 = await new Game().get_adminsetData(adminset);
...
async function get_ranking_power_img(e, Data, usr_paiming, thisplayer) {
    ...
    const data1 = await new Game().get_ranking_powerData(ranking_power_data);
...
async function get_ranking_money_img(e, Data, usr_paiming, thisplayer, thisnajie) {
    ...
    const data1 = await new Game().get_ranking_moneyData(ranking_money_data);
...
async function get_ningmenghome_img(e, thing_type) {
    ...
    const data1 = await new Game().get_ningmenghomeData(ningmenghome_data);
...
async function get_valuables_img(e) {
    ...
    const data1 = await new Game().get_valuablesData(valuables_data);
...
async function get_danfang_img(e) {
    ...
    const data1 = await new Game().get_danfangData(danfang_data);
...
async function get_tuzhi_img(e) {
    ...
    const data1 = await new Game().get_tuzhiData(tuzhi_data);
...
async function player_efficiency(usr_qq) {
    ...
    if (Math.floor(player.修炼效率提升) != Math.floor(player.修炼效率提升)) {
        player.修炼效率提升 = 0;
    }
    player.修炼效率提升 =
        linggen_efficiency +
...
async function get_supermarket_img(e, thing_class) {
    ...
    const data1 = await new Game().get_supermarketData(supermarket_data);
...
async function get_forum_img(e, thing_class) {
    ...
    const data1 = await new Game().get_forumData(forum_data);
