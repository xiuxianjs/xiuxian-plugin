        ...
        switch (record.type) {
            ...
            case RechargeType.BIG_MONTH_CARD:
                ...
                if (userInfo.small_month_card_expire_time > Date.now()) {
                    userInfo.small_month_card_expire_time = userInfo.big_month_card_expire_time + (userInfo.small_month_card_expire_time - Date.now());
                }
                for (const gift of BIG_MONTH_CARD_RECHARGE_GIFTS) {
                    await addNajieThing(userId, gift.name, gift.type, gift.account);
                }
                break;
            case RechargeType.COMBO:
                userInfo.currency += record.currency_gained;
                if (record.month_card_type === 'big') {
                    userInfo.big_month_card_expire_time = Math.max(Date.now(), userInfo.big_month_card_expire_time) + MONTH_CARD_CONFIG.BIG.days * 24 * 60 * 60 * 1000;
                    if (userInfo.small_month_card_expire_time > Date.now()) {
                        userInfo.small_month_card_expire_time = userInfo.big_month_card_expire_time + (userInfo.small_month_card_expire_time - Date.now());
                    }
...
const isUserMonthCardByMothData = (userInfo, cardType) => {
    if (!userInfo) {
        return false;
    }
    if (!cardType) {
...
const isUserMonthCard = async (userId, cardType) => {
    try {
...
async function addUserCurrency(userId, currency) {
    if (currency <= 0) {
...
export { CURRENCY_CONFIG, MONTH_CARD_CONFIG, PaymentStatus, RECHARGE_TIERS, REDIS_KEYS, RechargeType, addUserCurrency, calculateCurrencyGained, calculateFirstRechargeBonus, checkUserMonthCardStatus, completeRechargePayment, consumeMonthCardDays, consumeUserCurrency, createRechargeRecord, findUserRechargeInfo, getAllRechargeRecords, getAllUsersCurrencyInfo, getAmountByTier, getGlobalRechargeStats, getNextRechargeRecordId, getRechargeRecordDetail, getUserRechargeRecords, isTransactionIdExists, isUserMonthCard, isUserMonthCardByMothData, rechargeUserBigMonthCard, rechargeUserCurrency, rechargeUserSmallMonthCard };
