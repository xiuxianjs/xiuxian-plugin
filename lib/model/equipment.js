...
async function readEquipment(usrId) {
    return await getDataJSONParseByKey(keys.equipment(usrId));
}
async function writeEquipment(usrId, equipment) {
    ...
    player.攻击 = Number(levelInfo?.基础攻击 ?? 0) + Number(player.攻击加成 ?? 0) + Number(physiqueInfo?.基础攻击 ?? 0);
    player.防御 = Number(levelInfo?.基础防御 ?? 0) + Number(player.防御加成 ?? 0) + Number(physiqueInfo?.基础防御 ?? 0);
    player.血量上限 = Number(levelInfo?.基础血量 ?? 0) + Number(player.生命加成 ?? 0) + Number(physiqueInfo?.基础血量 ?? 0);
