...
async function readEquipment(usr_qq) {
    ...
    if (!equipment)
        return null;
    return safeParse(equipment, null);
}
async function writeEquipment(usr_qq, equipment) {
    ...
    const physiqueInfo = data.LevelMax_list.find(item => item.level_id == player.Physique_id);
    player.攻击 =
    ...
    await redis.set(`${__PATH.equipment_path}:${usr_qq}`, JSON.stringify(equipment));
}
