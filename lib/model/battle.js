...
async function zdBattle(AplayerA, BplayerB) {
    const normalize = (p) => {
        return {
            名号: p.名号,
    ...
    let playerA = normalize(BplayerB);
    let playerB = normalize(AplayerA);
    let cnt = 0;
    let A_xue = 0;
    let B_xue = 0;
    let t;
...
var battle = { zdBattle, baojishanghai, Harm, kezhi, ifbaoji };

export { Harm, baojishanghai, battle as default, ifbaoji, kezhi, zdBattle };
