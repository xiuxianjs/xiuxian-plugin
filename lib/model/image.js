...
async function getGongfaImage(e) {
    ...
    const xuexi_gongfa = Array.isArray(rawXuexi) ? rawXuexi.filter(v => typeof v === 'string') : [];
    const gongfa_have = [];
    const gongfa_need = [];
...
async function getPlayerImage(e) {
    ...
    const rawXuexi = player.学习的功法;
    const learned_gongfa = Array.isArray(rawXuexi) ? rawXuexi.filter(v => typeof v === 'string') : [];
    ...
    const power = ((player.攻击 * 0.9 + player.防御 * 1.1 + player.血量上限 * 0.6 + player.暴击率 * player.攻击 * 0.5 + Number(player.灵根?.法球倍率 || 0) * player.攻击)
        / 10000).toFixed(2);
