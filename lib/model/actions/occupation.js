...
import '../currency.js';
import { addExp4 } from '../xiuxian_m.js';
import 'crypto';
import 'posthog-node';
import '../message.js';
...
async function plantJiesuan(user_id, time, group_id) {
...
async function mineJiesuan(user_id, time, group_id) {
    const userId = user_id;
    const player = await getDataJSONParseByKey(keys.player(userId));
    if (!player) {
        return false;
...
export { calcEffectiveMinutes, mineJiesuan, plantJiesuan };
