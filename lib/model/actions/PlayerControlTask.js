...
const calculateCultivationEfficiency = (actualTime) => {
    ...
    const growthFactor = Math.log(1 + excessTime / 30) / Math.log(2);
    const efficiency = 1 + growthFactor * 0.5;
    return efficiency;
};
const calculateWorkEfficiency = (actualTime) => {
    ...
    const excessTime = timeMinutes - optimalMinutes;
    const growthFactor = Math.log(1 + excessTime / 15) / Math.log(2);
    const efficiency = 1 + growthFactor * 0.3;
    return efficiency;
};
