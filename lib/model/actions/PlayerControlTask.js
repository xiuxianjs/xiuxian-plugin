...
const calculateCultivationEfficiency = (actualTime) => {
    ...
    const decayFactor = Math.log(1 + excessTime / 60) / Math.log(2);
    const efficiency = Math.max(0.1, 1 - decayFactor * 0.3);
    return efficiency;
};
const calculateWorkEfficiency = (actualTime) => {
    ...
    const excessTime = timeMinutes - optimalMinutes;
    const decayFactor = Math.log(1 + excessTime / 30) / Math.log(2);
    const efficiency = Math.max(0.1, 1 - decayFactor * 0.4);
    return efficiency;
};
