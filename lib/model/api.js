...
const verc = ({ e }) => {
    const { whitecrowd, blackid } = config.getConfig('parameter', 'xiuxian');
    if (Array.isArray(whitecrowd) &&
        whitecrowd.indexOf(e.group_id) === -1)
        return false;
    if (Array.isArray(blackid) && blackid.indexOf(e.UserId) !== -1)
        return false;
    return true;
};
