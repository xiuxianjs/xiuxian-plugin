...
import { zdBattle, Harm } from './battle.js';
import { sleep } from './common.js';
import { addHP, addCoin } from './economy.js';
import { keysByPath, __PATH, keys, getRedisKey } from './keys.js';
import { readAction, isActionRunning, formatRemaining, remainingMs } from '../response/actionHelper.js';
import '@alemonjs/db';
import './DataList.js';
import { existplayer } from './xiuxian_impl.js';
import data from './XiuxianData.js';
import { KEY_WORLD_BOOS_STATUS_TWO, KEY_RECORD_TWO, KEY_AUCTION_GROUP_LIST, KEY_WORLD_BOOS_STATUS, KEY_RECORD } from './constants.js';
import './settions.js';
import 'jsxp';
import 'md5';
import 'react';
...
async function InitWorldBoss() {
    ...
    const Health = Math.trunc(X * 150 * player_quantity * 2);
...
async function Boss2IsAlive() {
    return ((await redis.get(KEY_WORLD_BOOS_STATUS_TWO)) &&
        (await redis.get(KEY_RECORD_TWO)));
}
const BossIsAlive = Boss2IsAlive;
...
async function WorldBossBattle(e) {
    ...
    if (await data.existData('player', usr_qq)) {
        ...
        if (WorldBOSSBattleCD[usr_qq]) {
            const Seconds = Math.trunc((300000 - (Date.now() - WorldBOSSBattleCD[usr_qq])) / 1000);
        ...
        if (+PlayerRecord == 0) {
            const QQGroup = [], DamageGroup = [], Name = [];
            QQGroup[0] = usr_qq;
            DamageGroup[0] = 0;
            Name[0] = player.名号;
            PlayerRecordJSON = {
        ...
        if (WorldBossStatus.Health <= 0) {
            ...
            for (let i = 0; i < PlayerList.length; i++) {
                const CurrentPlayer = await data.getData('player', PlayerRecordJSON.QQ[PlayerList[i]]);
                if (i < Show_MAX) {
