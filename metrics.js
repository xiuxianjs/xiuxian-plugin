...
if (enableMetrics) {
  ...
  function classifyCpu(v) {
    // CPU 百分比(已平摊多核)。
    if (v < 20) return { level: 'good', desc_zh: 'CPU负载低', desc_en: 'Low CPU usage' };
    if (v < 70)
      return {
        level: 'moderate',
        desc_zh: 'CPU适中',
  ...
  function classifyRss(pct) {
    ...
    if (pct < 5) return { level: 'moderate', desc_zh: 'RSS正常', desc_en: 'Normal RSS' };
    if (pct < 10) return { level: 'high', desc_zh: 'RSS较高', desc_en: 'High RSS' };
    return {
      level: 'critical',
      desc_zh: 'RSS过高需排查',
  ...
  setInterval(() => {
    try {
      ...
      function fmtStatus(s) {
        ...
        if (bilingual) return wrapColor(s.level, `${lvlZh}/${lvlEn}/${s.desc_zh}/${s.desc_en}`);
      ...
      if (mode === 'json') {
        ...
        const table =
          ...
          '\n' +
          pad('Heap Used', formatBytes(m.heapUsed) + ' ' + fmtStatus(statHeap)) +
          '\n' +
          pad('Heap Total', formatBytes(m.heapTotal)) +
          '\n' +
          pad('Heap Usage%', data.heap_usage_pct + ' %') +
          '\n' +
          pad('RSS', formatBytes(m.rss) + ` (${data.rss_pct_of_sysmem}%) ` + fmtStatus(statRss)) +
          '\n' +
          pad('External', formatBytes(m.external)) +
          '\n' +
          pad('ArrayBuffers', formatBytes(m.arrayBuffers || 0)) +
          '\n' +
          pad('Handles', `${handles} (${handleDelta >= 0 ? '+' : ''}${handleDelta})`) +
          '\n' +
          pad('Requests', `${requests} (${requestDelta >= 0 ? '+' : ''}${requestDelta})`) +
          '\n' +
